...
(function () {
   ...
   * session_ready, which confirms the server-side Gemini Live session is ready
  ...
  function _startVoiceInterview(selectedRole, userName) {
    ...
    ws.addEventListener('open', () => {
      setConnectionStatus('connected');
      // Send start frame immediately — AudioContext + mic start on session_ready.
      wsSend({
        type: 'start',
        data: { role: selectedRole, candidate_name: userName, mode: 'voice' },
      });
    });
  ...
  function handleVoiceWSMessage(event) {
    ...
    switch (type) {
      ...
      case 'session_ready':
        // Server has established the Gemini Live session.
      ...
      case 'ai_interrupted':
        ...
        // Gemini Live stopped generating — flush our playback queue so the
  ...
  /**
   * Called when the server confirms the Gemini Live session is ready.
   ...
   *   - sampleRate: 16000       — Gemini Live requires 16 kHz mono PCM16
   *   - channelCount: 1 (mono)
   *
   ...
   * @param {number}       targetSampleRate Target rate required by Gemini (16000)
   * @returns {Int16Array}
   */
